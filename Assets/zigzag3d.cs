...
public class zigzag3d : MonoBehaviour {
...
public static float Sigma(float Diff_Const)
	...
	float randomVel(){
		return (Mathf.Sqrt(Time.deltaTime) * NextGaussian()* Sigma(D))/Time.deltaTime;
	}
	...
	void FixedUpdate () {
		...
		if(count == No_Frames){
			...
			Vector3 Increment_Vel = new Vector3((float)10e4*randomVel()/Mathf.Sqrt(No_Frames),(float)10e4*randomVel()/Mathf.Sqrt(No_Frames),(float)10e4*randomVel()/Mathf.Sqrt(No_Frames));
			Vector3 New_Vel = Old_Vel + Increment_Vel;
